...
class ChainPosController(Node):
    ...
    def chain_pos_callback(self, msg):
        ...
        if width > 20:  # Assuming 20 is the threshold below which the object is considered out of view
            ...
            # Additional logic for sway based on current_yaw and target_range
            if self.target_range[0] <= self.current_yaw <= self.target_range[1]:
                # Switch target range
                if self.target_range == (170, 180):
                    self.target_range = (100, 110)
                    sway = -self.sway_gain  # Use negative sway_gain when target is 80 to 90
                else:
                    self.target_range = (0, 10)
                    sway = self.sway_gain   # Use sway_gain when target is 0 to 10
            else:
                # Keep sway based on the last target range
                sway = self.sway_gain if self.target_range == (0, 10) else -self.sway_gain
        else:
...
#Sway is controlled based on a compass. When position is reached, next position is 180 deg oposite 
